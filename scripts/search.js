...
export function getSelectedProductCategory() {
 //const sku = window.location.pathname.split('/');
 return "Primary Antibodies";
}

async function getFullResponse(sku, selectedProductCategory) {
  if (!isFetchCalled) {
    let body = {};
 ...
 */
/* eslint consistent-return: off */
export async function getProductsListResponse() {
  try {
    let response;
    const productCategory = getSelectedProductCategory();
    const fullResponse = await getFullResponse(null, productCategory);

    if (fullResponse.results.length > 0) {
      response = fullResponse.results;
      //localStorage.setItem('optimus-product-details', JSON.stringify(fullResponse.results));
      return JSON.stringify(fullResponse.results);
    }
    await redirect404();
  } catch (error) {
    // eslint-disable-next-line no-console
